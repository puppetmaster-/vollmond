...
use keyframe::functions::{EaseOut, EaseIn};
    ...
    pub fn run(&mut self) -> Option<MainState>{
        ...
        if self.show_text1{
            for (i, line) in self.text1.split("\n").enumerate(){
                draw_text_ex(line, (screen_width()/2.0)-330.0, (screen_height()/2.0) - 350.0 + i as f32 * 80.0, tp);
            }
        }else{
            for (i, line) in self.text2.split("\n").enumerate(){
                draw_text_ex(line, (screen_width()/2.0)-290.0, (screen_height()/2.0) - 350.0 + i as f32 * 80.0, tp);
            }
        }

        if self.timer.finished(){
            draw_text_ex("press any key",(screen_width()/2.0)-180.0,(screen_height()/2.0) + 250.0 + self.tween.value(),TextParams{
                font: self.font,
                font_size: 100,
                font_scale: 0.5,
                color: FONT_COLOR
